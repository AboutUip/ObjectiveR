...
public final class RuntimeExecutor {
    ...
    private record RtResult(boolean returned, Value value) {
        static RtResult none() {
            return new RtResult(false, null);
        }

        static RtResult ret(Value v) {
            return new RtResult(true, v);
        }
    }
    ...
    private RtResult executeStmtWithoutBlock(
        ...
        switch (s) {
            ...
            case Stmt.Return ret -> {
                ...
                return RtResult.ret(widenReturnValue(declaredRet, raw));
            }
    ...
    private Value evalExpr(Expr e, Env env, StaticStore statics, String callerFile) {
        ...
        if (e instanceof Expr.Binary b) {
            ...
            return switch (b.op()) {
                case ADD -> evalArithmetic(left, right, 0);
                case SUB -> evalArithmetic(left, right, 1);
                case MUL -> evalArithmetic(left, right, 2);
                case DIV -> evalDiv(left, right);
                case MOD -> evalMod(left, right);
                case POW -> evalPow(left, right);
            };
        }
    ...
    private Value evalDiv(Value left, Value right) {
        ...
        return switch (left.type()) {
            case INT -> {
                int b = right.intValue();
                if (b == 0) {
                    throw new ObrException(E_RT_EXPR_UNSUPPORTED + " 除数为 0");
                }
            ...
            case FLOAT -> {
                ...
                yield Value.ofFloat(left.floatValue() / b);
            }
            case DOUBLE -> {
                double b = right.doubleValue();
                if (b == 0.0) {
                ...
                yield Value.ofDouble(left.doubleValue() / b);
            }
    ...
    private Value evalMod(Value left, Value right) {
        ...
        return switch (left.type()) {
            case INT -> {
                int b = right.intValue();
                if (b == 0) {
                    throw new ObrException(E_RT_EXPR_UNSUPPORTED + " 取模除数为 0");
                }
                yield Value.ofInt(left.intValue() % b);
            }
            case LONG -> {
    ...
    private Value evalPow(Value left, Value right) {
