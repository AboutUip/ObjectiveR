...
| [lexing.md](lexing.md) | `Lexer`、`Token`、`TokenKind`、`CharLiteralParser`（含 `''`） |
| [parsing.md](parsing.md) | `Parser`：`parseAssignment`/`parseConditional`、语句与表达式语句 |
...
| [semantic-binding.md](semantic-binding.md) | `SemanticBinder`：`checkAssignSemantics`、`checkExprStmtExpr`、推断与循环/`break`/`continue` |
| [overload-resolution.md](overload-resolution.md) | `NumericWidening`、语义与运行时重载 |
| [execution.md](execution.md) | `RuntimeExecutor`：`evalExpr`/`evalAssignExpression`/`evalExprStmtDiscard`、`call`、TCO、栈、`std::rout` |
| [runtime-model.md](runtime-model.md) | `RuntimeExecutor` 内部 `Value`/`Env`/`StaticStore`/栈 |
| [architecture.md](architecture.md) | 子包索引与数据流 |
| [implementation-scope.md](implementation-scope.md) | 相对全集：表达式语句、`Expr.Assign`、`?:` 分支赋值、`string` 引用相等与字面量不驻留、`''`、`while`/`break`/`continue` 等 |
